...
@component('admin::components.page.header')
    @slot('title', 'Edit Delivery Charge', ['resource' => 'Edit Delivery Charge'])
    @slot('subtitle', $delivery_charge->charge)

    <li><a href="{{ route('admin.delivery.charge') }}">{{ __('Delivery Charge') }}</a></li>
    <li class="active">{{ __('Edit Delivery Charge') }}</li>
@endcomponent

@section('content')
    <form method="POST" action="{{ route('admin.delivery.charge.update', $delivery_charge) }}" class="form-horizontal"
        id="delivery-charge-edit-form" novalidate>
        {{ csrf_field() }}
        {{ method_field('put') }}
                                                ...
                                                <select name="city_id" class="form-control" id="city_id">
                                                    ...
                                                    @foreach ($cities as $city)
                                                        <option value="{{ $city->id }}"
                                                            {{ $delivery_charge->city_id == $city->id ? 'selected' : '' }}>
                                                            {{ $city->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="col-md-9">
                                                <div class="checkbox">
                                                    <input type="checkbox" name="status" class="" id="status"
                                                        value="1" {{ $delivery_charge->status ? 'checked' : '' }}>
