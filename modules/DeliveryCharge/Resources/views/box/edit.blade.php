...
@component('admin::components.page.header')
    @slot('title', 'Edit Box Charge', ['resource' => 'Edit Box Charge'])
    @slot('subtitle', $box_charge->charge)

    <li><a href="{{ route('admin.box.charge') }}">{{ __('Box Charge') }}</a></li>
    <li class="active">{{ __('Edit Box Charge') }}</li>
@endcomponent

@section('content')
    <form method="POST" action="{{ route('admin.box.charge.update', $box_charge) }}" class="form-horizontal"
        id="box-charge-edit-form" novalidate>
        {{ csrf_field() }}
        {{ method_field('put') }}
                                            ...
                                            <div class="col-md-9">
                                                <div class="checkbox">
                                                    <input type="checkbox" name="status" class="" id="status"
                                                        value="1" {{ $box_charge->status ? 'checked' : '' }}>
