...
class Money implements JsonSerializable
{
    ...
    public function format($currency = null, $locale = null)
    {
        ...
        // Ensure $this->amount is numeric
        $amount = is_numeric($this->amount) ? (float) $this->amount : 0.0;

        // Initialize NumberFormatter for currency formatting
        $numberFormatter = new NumberFormatter($locale, NumberFormatter::CURRENCY);

        // Format the amount
        $formattedAmount = $numberFormatter->formatCurrency($amount, $currency);

        // Remove any non-breaking space (\u{A0}) if it exists
        $formattedAmount = str_replace("\u{A0}", ' ', $formattedAmount);

        // Fix: Remove decimals for Hungarian Forint (HUF), as it's a non-decimal currency
        if ($currency === 'HUF') {
            $formattedAmount = preg_replace('/,00$/', '', $formattedAmount);
        }
