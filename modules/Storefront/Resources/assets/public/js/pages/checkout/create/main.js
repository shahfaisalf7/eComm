...
Alpine.data(
    ...
    ({
        ...
        get hasBillingCities() {
            return Object.keys(this.cities.billing).length !== 0;
        },

        get hasBillingZones() {
            return Object.keys(this.zones.billing).length !== 0;
        },
        ...
        get hasShippingCities() {
            return Object.keys(this.cities.shipping).length !== 0;
        },

        get hasShippingZones() {
            return Object.keys(this.zones.shipping).length !== 0;
        },
        ...
        init() {
            ...
            this.$watch("form.billing.zone", (newZone) => {
                if (newZone) {
                    // this.addTaxes();
                }
            });

            this.$watch("form.shipping.zone", (newZone) => {
                if (newZone) {
                    // this.addTaxes();
                }
            });
        ...
        addNewBillingAddress() {
            ...
            var country = this.firstCountry;
            this.changeBillingCountry(country);
        },
        ...
        addNewShippingAddress() {
            ...
            var country = this.firstCountry;
            this.changeShippingCountry(country);
        },

        loadMetaDataIfNoAddressFound() {
            if (!this.hasAddress) {
        ...
        changeBillingZones(zone) {
            this.form.billing.zone = zone;
        },

        changeShippingZones(zone) {
            this.form.shipping.zone = zone;
        },
        ...
        changeBillingState(state) {
            ...
            this.fetchCities(state, (response) => {
                this.cities.billing = response.data;
                this.form.billing.city = "";
            });
        },

        changeBillingCities(city) {
            this.form.billing.city = city;

            if (city === "") {
                this.form.billing.zone = "";
                this.zones.billing = {};

                return;
            }

            this.fetchZones(city, (response) => {
                this.zones.billing = response.data;
                this.form.billing.zone = "";
            });
        },

        changeShippingCities(city) {
            this.form.shipping.city = city;

            if (city === "") {
                this.form.shipping.zone = "";
                this.zones.shipping = {};
            ...
            this.fetchZones(city, (response) => {
                this.zones.shipping = response.data;
                this.form.shipping.zone = "";
            });
        },
        ...
        fetchCities(state, callback) {
            axios
                .get(route("states.cities.index", { code: state }))
                .then(callback);
        },

        fetchZones(city, callback) {
            axios
                .get(route("cities.zones.index", { code: city }))
                .then(callback);
        },
        ...
        changeShippingState(state) {
            ...
            if (state === "") {
                ...
                this.cities.shipping = {};

                return;
            }

            this.fetchCities(state, (response) => {
                this.cities.shipping = response.data;
                this.form.shipping.city = "";
            });
        },
        ...
        async deleteOrder(orderId) {
            ...
            const response = await axios.get(
                route("checkout.payment_canceled.store", { orderId })
            );
        ...
        async renderStripeElements() {
            ...
            this.stripeElements = this.stripe.elements({
                mode: "payment",
                amount: Math.round(this.$store.state.cartTotal * 100),
