...
$_SITEID   = '43922276';
$_TOKEN    = 'a377ae21ca70905250253c093bbab9e5';
$_UNIQUE   = 'a27b47d679d28cffc6bb4e943a3afc0a';
...
// we'll need single id for second step of check feature to request encryption details
...
if ( $_FEATURECODE == DBSCAN && $_PLATFORM == 'wordpress' )
{
    ...
    try {
        $db = getDbObj( true );
        if ( !empty($prefixes = $db->find_distinct_WP_prefixes()) ) {
            // appears to be multisite compatible, but only log for now, since we'll limit tables scanned only to the ones with prefix in config
            add_to_log( $prefixes, 'WP multisite check: which distinct prefixes were found?' );
        }
    }
    catch( Exception $ex)
    {
        add_to_log( false, 'Failed to run find_distinct_WP_prefixes (not critical)' );
    }

    add_to_log( $statuses['singlesite'] ? 'yes (no WP_ALLOW_MULTISITE config found)' : 'NO', 'Check Features - single site?' );
}
...
function test_mysqldump_with_shell( $backup_schemas, &$statuses )
{
    ...
    {
        $statuses['mysqldump'] = false;
        add_to_log( false, 'failed to access mysql' );
    }
    ...
    {
        $statuses['mysqldump'] = false;
        add_to_log( false, 'failed to access mysqldump' );
    }


    if ( !$statuses['mysqldump'] )
    {
        // if either command failed, we can reset the status and try using software solution that does not rely on command line
        $statuses['mysqldump'] = true;
        return test_mysqldump_no_shell( $backup_schemas, $statuses );
    }
    else
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $command = "{$mysqldump} --defaults-file={$config_file} -h" . $host . $port . " --single-transaction --events --routines --force --no-data --no-tablespaces {$schemas_flag} > /dev/null";
